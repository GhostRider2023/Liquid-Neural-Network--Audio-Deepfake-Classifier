...
def make_for_loader(for_root: str, batch_size: int = 512, split: str = 'all'):
    # Use the specified split of FOR dataset
    ds = FORDataset(for_root, split=split)
...
def main():
    ...
    parser.add_argument('--la_save_root', type=str, default='/kaggle/input/mlaad_train_weights/pytorch/default/1/FOR_logs_Final', help='Directory containing repeat* folders with LA checkpoints')
    ...
    for dir_name in candidate_dirs:
        ...
        model = LiqNNModel(input_size=128, hidden_size=32, out_size=1, time_steps=150).to(device)
