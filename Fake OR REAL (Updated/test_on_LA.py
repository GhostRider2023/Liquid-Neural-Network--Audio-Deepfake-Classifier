...
class LADataset(Dataset):
    def __init__(self, la_root, split='test', sample_rate=16000, n_mels=128, n_fft=1024, hop_length=160, duration=10):
        self.items = []
        ...
        if not os.path.exists(protocol_file):
            protocol_file = os.path.join(la_root, 'ASVspoof2019_LA_cm_protocols', f'ASVspoof2019.LA.cm.{split}.trn.txt')
        
        with open(protocol_file, 'r') as f:
            for line in f:
                parts = line.strip().split()
                if len(parts) >= 5:
                    audio_file, key = parts[1], parts[4]
                    label = 1 if key == 'bonafide' else 0
                    self.items.append((audio_file, label))
        
        self.flac_dir = os.path.join(la_root, f'ASVspoof2019_LA_{split}', 'flac')

    def __len__(self): return len(self.items)
    def __getitem__(self, idx):
        audio_id, label = self.items[idx]
        path = os.path.join(self.flac_dir, audio_id + '.flac')
...
def eval_at_threshold(model, loader, device, threshold=0.5):
    ...
    y_true, y_score = [], []
    with torch.no_grad():
        for mel, label in tqdm(loader, desc="Evaluating LA"):
            mel = mel.to(device)
            out = model(mel)
...
def main():
    ...
    parser.add_argument('--la_root', type=str, default=r"c:\Users\Shivaay Dhondiyal\Desktop\shivaay\coding\2_projects\9_deepfake_paper\LA", help='LA dataset root')
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--out_log', type=str, default='LA_eval_results.txt')
    ...
    dataset = LADataset(args.la_root, split='eval') # Use eval for final test
    ...
    print(f"LA Eval Done: Acc={acc:.4f} EER={eer:.4f} AUC={auc:.4f}")
